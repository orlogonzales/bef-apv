...
<head>
	...
	<script type="text/javascript">
		$(document).ready(function() {
			...
			$('#terminarActividad').click(function(){
				event.preventDefault();
				var operacion = "terminarActividad";
						...
						if (!isConfirm) {
							$.ajax({
								...
								success: function(respuesta){
									...
									var urlSistemaTerminarActividad = '<?= $urlSistemaTerminarActividad ?>';
									if(respuesta.resultado=='OK'){
										window.location.replace(urlSistemaTerminarActividad);
									}
...
<div id="fullscreen">
	<div class="navbar navbar-inverse">
		...
		<button type="button" id="terminarActividad" class="btn btn-xs btn-danger pull-right" style="margin-top: 5px !important;">TERMINAR ACTIVIDAD</button>
	</div>
						...
						<?php if($diasEntre==0){ ?>
							<div class="col-lg-6 col-lg-offset-3">
								<div class="panel panel-body border-top-xlg border-top-warning-800 text-center">
									...
									<h5 style="margin-bottom: 10px; padding-bottom: 10px; border-bottom: 1px solid #CCC;"><a href="<?= $urlSistemaActividadesAsistencia ?>" class="btn btn-primary btn-md" style="font-size: 15px;">REGISTRAR CON <strong style="font-weight: bolder;">TARJETA</strong></a></h5>
									...
									<script type="text/javascript">
										$(document).ready(function(){
											...
											///////////////////////////////////////////////////
											$("#DNISocio").on( "keypress", function(event) {
												if (event.which == 13 && !event.shiftKey) {
													event.preventDefault();
													var codigoActividad = '<?= $codigoActividad ?>';
													var formaActividad  = '<?= $formaActividad ?>';
													var operacion       = 'REGISTRA_ASISTENCIA_DNI';
													var DNISocio     = $('input#DNISocio').val();
													var datos           = 'codigoActividad='+codigoActividad+'&DNISocio='+DNISocio+'&operacion='+operacion;
													if(formaActividad==1){
														var ruta= '<?= $urlSistema ?>api/index.php';
														$.ajax({
															type: "POST",
															...
															success: function(respuesta){
																...
																if(respuesta.estado=="registrado"){
																	$.jGrowl('SOCIO REGISTRO SU INGRESO.', { header: 'CONFIRMACION', theme: 'alert-styled-left bg-success-300' });
																	window.location.replace("<?= $urlSistemaActividadesAsistencia ?>");
																}
																...
																if(respuesta.estado=="ya_registro_ingreso"){
																	$.jGrowl('YA REGISTRO SU INGRESO.', { header: 'CONFIRMACION', theme: 'alert-styled-left bg-warning-300' });
																}
														...
														$.ajax({
															...
															dataType:'json',
															success: function(respuesta){
																if(respuesta.mensaje=="INGRESO_REGISTRADO"){
																	location.reload();
																}
																...
																if(respuesta.mensaje=="SOCIO_REGISTRADO"){
									...
									<div class="form-group mt-20">
										<input type="text" id="DNISocio" maxlength="8" class="form-control text-center registroCodigo text-danger" placeholder="Ingrese DNI" onblur="this.focus();">
										<div id="print"></div>
									</div>
								</div>
							</div>
