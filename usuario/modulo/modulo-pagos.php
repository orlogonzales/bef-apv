...
<?php if($opcion=="modulo_pago_monto"){ ?>
	<script type="text/javascript">
		$(document).ready(function(){
			...
			$("button#bt_pagar_monto"+codigoConcepto).click(function(){
				...
				var idJuntaDirectiva = '<?= $idJuntaDirectiva ?>';
				var valida           = $('.form_valida_pago_completo').valid();
				var datos            = {
					operacion        : operacion,
					codigoSocio      : codigoSocio,
					tipoActividad    : conceptoPago,
		...
		function verificaVOU(){
			...
			if($.trim(nroDoc).length>0){
				$.ajax({
					...
					data: datos,
					dataType:'json',
					beforeSend: function(){ $('#infoVOU').html('<i class=" icon-spinner3"></i>'); },
					success: function(respuesta){
	...
	<?php
		if($conceptoPago==CUO){
			$sql="SELECT conceptoCuota, fechaPago FROM sm_mod_cuotas WHERE codigoCuota='$codigoConcepto'";
			$row=mysqli_query($conexion,$sql);
			$dato=mysqli_fetch_array($row);
			$conceptoPago=$dato['temaActividad'];
			$fechaActividad=$dato['fechaActividad'];
			$etiqueta='PAGAR DESDE';
		}else{
			$sql="SELECT temaActividad, fechaActividad FROM sm_mod_actividades WHERE codigoActividad = '$codigoConcepto'";
			$row=mysqli_query($conexion,$sql);
			$dato=mysqli_fetch_array($row);
			$conceptoPago=$dato['temaActividad'];
			$fechaActividad=$dato['fechaActividad'];
			$etiqueta='FECHA ACTIVIDAD';
		}
		
		$nombreSocio=infoSocios($codigoSocio,'nombreLargo');
	?>
	<form class="form_valida_pago_completo">
		<div class="row mb-15">
			<div class="col-md-6">
				<ul class="list-group">
					<li class="list-group-item"><strong>SOCIO:</strong> <?= $nombreSocio ?></li>
					<li class="list-group-item"><strong>LOTES:</strong> <?= $lotes ?></li>
				</ul>
			</div>
			<div class="col-md-6">
				<ul class="list-group">
					<li class="list-group-item"><strong>CONCEPTO:</strong> <?= $conceptoPago ?></li>
					<li class="list-group-item text-uppercase"><strong><?= $etiqueta ?></strong> <?= infoFecha($fechaActividad,'larga') ?></li>
				</ul>
			</div>
		</div>
		...
		<div class="form-group">
			<div class="row">
				<div class="col-sm-3">
					<div class="input-group">
						...
						<span class="form-control input-lg textoNegrita text-uppercase"><?= infoFecha($fechaActividad,'normal') ?></span>
					</div>
					<span class="label label-block label-default text-left tope2">Pagar desde</span>
				</div>
				<div class="col-sm-3">
					<div class="input-group">
			...
			<div class="row border-top border-top-gray-1 mt-20">
				<div class="col-sm-4"></div>
				<div class="col-sm-4 mt-15">
					<input type="hidden" name="idJuntaDirectiva" id="idJuntaDirectiva" value="<?= $idJuntaDirectiva ?>">
	...
	<script type="text/javascript">
		$(document).ready(function(){
			...
			$("button#bt_volver_pagos").click(function(){
				...
				$('#modulo_pago-'+codigoSocio).fadeIn("slow").html('<div class="form-group"><div class="row"><div class="col-sm-12 text-center"><img src="'+ruta+'assets/images/loader.gif"><br>CARGANDO MODULO DE PAGO</div></div></div>').fadeIn(1000).delay(1000);
				$("#modulo_pago-"+codigoSocio).fadeIn("slow").load('modulo/modulo-pagos.php?'+datos).fadeIn(1000).delay(1000);
				$("#modulo_fechas_pago-"+codigoSocio).hide();
			});
			...
			$("button#bt_programar_nro_cuotas").click(function(){
				...
				$('#modulo_fechas_pago-'+codigoSocio).fadeIn("slow").html('<div class="form-group"><div class="row"><div class="col-sm-12 text-center"><img src="'+ruta+'assets/images/loader.gif"><br>CARGANDO MODULO DE PAGO</div></div></div>').fadeIn(1000).delay(1000);
				$("#modulo_fechas_pago-"+codigoSocio).fadeIn("slow").load('modulo/modulo-pagos.php?'+datos).fadeIn(1000).delay(1000);
			});
		...
		<div class="form-group">
			<div class="row">
				<div class="col-sm-3">
					...
					<span class="label label-block label-default text-left tope2">Monto a programar</span>
				</div>
				<div class="col-sm-5">
					<div class="input-group">
					...
					$.ajax({
						...
						success: function(respuesta){
							if(respuesta.mensaje=="PROGRAMACION_FINALIZADA"){
								new PNotify({title: 'CONFIRMACION', text: 'La cuenta fue programada en '+cuotasPago+' cuotas de pago.', addclass: 'bg-success'});
								var opcion='modulo_pago_monto';
								var datos='opcion='+opcion+'&codigoSocio='+codigoSocio+'&conceptoPago='+conceptoPago+'&codigoConcepto='+codigoConcepto+razon;
								$('#modulo_pago-'+codigoSocio).fadeIn("slow").html('<div class="form-group"><div class="row"><div class="col-sm-12 text-center"><img src="'+ruta+'assets/images/loader.gif"><br>CARGANDO MODULO DE PAGO</div></div></div>').fadeIn(1000).delay(1000);
								$("#modulo_pago-"+codigoSocio).fadeIn("slow").load('modulo/modulo-pagos.php?'+datos).fadeIn(1000).delay(1000);
...
<?php if($opcion=="pagar_fechas_programadas"){ ?>
	...
	<?php
		...
		$sql="SELECT COUNT(cuota) as nroCuotas FROM sm_mod_cuentas WHERE codigoSocio='$codigoSocio' AND codigoConcepto='$codigoConcepto'";
		$row=mysqli_query($conexion,$sql);
		$dato=mysqli_fetch_array($row);
		$nroCuotas=$dato['nroCuotas'];

		$nombreSocio=infoSocios($codigoSocio,'nombreLargo');
		$conceptoPago=infoCuota('',$codigoConcepto,'conceptoCuota');
		$fechaPago=infoFecha($fechaActividad,'normal');
	?>
	...
	<div class="row mb-15">
		<div class="col-md-6">
			<ul class="list-group">
				<li class="list-group-item"><strong>SOCIO:</strong> <?= $nombreSocio ?></li>
				<li class="list-group-item"><strong>LOTES:</strong> <?= $lotes ?></li>
				<li class="list-group-item"><strong>TOTAL CUOTA:</strong> S/. <?= moneda($montoPago) ?></li>
			</ul>
		</div>
		<div class="col-md-6">
			<ul class="list-group">
				<li class="list-group-item"><strong>CONCEPTO:</strong> <?= $conceptoPago ?></li>
				<li class="list-group-item text-uppercase"><strong>PAGAR DESDE:</strong> <?= $fechaPago ?></li>
				<li class="list-group-item text-uppercase"><strong>PAGO FRACCIONADO:</strong> <?= ceros($nroCuotas,2) ?> CUOTAS</li>
			</ul>
		</div>
	</div>
	...
	<?php
		...
		while($n=mysqli_fetch_array($rs)){
			...
			$query="SELECT idJuntaDirectiva, fechaActividad FROM sm_mod_actividades WHERE codigoActividad = '$codigoConcepto' LIMIT 1";
